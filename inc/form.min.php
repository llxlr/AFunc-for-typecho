...
class PluginsForm{
	...
	static function LoginCard(){
		...
		$result = new Typecho_Widget_Helper_Form_Element_Text('LoginCard', NULL, NULL, _t('登录框美化'), _t('请分别填写背景颜色、字体颜色、透明度和圆角值，以 - 分割'));
	...
	static function Weather(){
		...
		$result = new Typecho_Widget_Helper_Form_Element_Text('Weather', NULL, NULL, _t('天气信息'), _t('填写高德API（目前只支持高德API，后面会慢慢增加)'));
	...
	static function RandomImg(){
		...
		$result = new Typecho_Widget_Helper_Form_Element_Text('RandomImg', NULL, NULL, _t('随机图'), _t('参数：name(必选，名称)、dir(可选，路径)，举例：name=as&dir=usr/api。填写参数后，请点击右下角小飞机激活改功能'));
